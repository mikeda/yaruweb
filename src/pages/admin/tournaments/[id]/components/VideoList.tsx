  ...
  return (
    <>
      ...
      <Menu id="video-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleClose}>
        <MenuItem
          onClick={() => {
            router.push(pagesPath.admin.tournament_videos._id(tournamentVideoId).edit.$url());
          }}
        >
          編集する
        </MenuItem>
        <MenuItem
          onClick={() => {
            router.push(pagesPath.admin.tournament_videos._id(tournamentVideoId).battles.$url());
          }}
        >
          対戦を登録する
        </MenuItem>
